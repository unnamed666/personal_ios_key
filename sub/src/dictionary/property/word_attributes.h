 ...
 public:
    ...
    WordAttributes()
            : mProbability(NOT_A_PROBABILITY), mIsBlacklisted(false), mIsNotAWord(false),
              mIsPossiblyOffensive(false), mTimestamp(NOT_A_TIMESTAMP), mNGramType(NOT_A_NGRAM_TYPE) {}
    ...
    WordAttributes(const int probability, const bool isBlacklisted, const bool isNotAWord,
            ...
            : mProbability(probability), mIsBlacklisted(isBlacklisted), mIsNotAWord(isNotAWord),
              mIsPossiblyOffensive(isPossiblyOffensive), mTimestamp(NOT_A_TIMESTAMP), mNGramType(NOT_A_NGRAM_TYPE) {}

    WordAttributes(const int probability, const bool isBlacklisted, const bool isNotAWord,
                   const bool isPossiblyOffensive, const int timestamp)
            : mProbability(probability), mIsBlacklisted(isBlacklisted), mIsNotAWord(isNotAWord),
              mIsPossiblyOffensive(isPossiblyOffensive), mTimestamp(timestamp), mNGramType(NOT_A_NGRAM_TYPE) {}

    WordAttributes(const int probability, const bool isBlacklisted, const bool isNotAWord,
                   const bool isPossiblyOffensive, const int timestamp, const int ngramType)
            : mProbability(probability), mIsBlacklisted(isBlacklisted), mIsNotAWord(isNotAWord),
              mIsPossiblyOffensive(isPossiblyOffensive), mTimestamp(timestamp), mNGramType(ngramType) {}
    ...
    void rescore(int newProbability) {
        mProbability = newProbability;
    }
