...
/* static */ bool BigramListReadWriteUtils::getBigramEntryPropertiesAndAdvancePosition(
        const ReadOnlyByteArrayView buffer, BigramFlags *const outBigramFlags, BigramFlags *const outBigramProbability,
    ...
    if(outBigramProbability) {
        const BigramFlags bigramProbability = ByteArrayUtils::readUint8AndAdvancePosition(buffer.data(), bigramEntryPos);
        *outBigramProbability = bigramProbability;
    }
...
/* static */ bool BigramListReadWriteUtils::skipExistingBigrams(const ReadOnlyByteArrayView buffer,
    ...
    BigramFlags flags;
    BigramFlags probability;
    bool useNewAlgorithm = AlgorithmSwitch::getInstance().isUseNewAlgorithm();

    do {
        if (!getBigramEntryPropertiesAndAdvancePosition(buffer, &flags, useNewAlgorithm ? &probability : nullptr, 0 /* outTargetPtNodePos */,
                bigramListPos)) {
            return false;
        }
    } while(hasNext(flags));
