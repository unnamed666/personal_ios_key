...
const WordAttributes LanguageModelDictContent::getWordAttributes(const WordIdArrayView prevWordIds,
    ...
    for (int i = maxPrevWordCount; i >= 0; --i) {
        ...
        if (mHasHistoricalInfo) {
            ...
            probability = getWordScore(historicalInfo, contextCount, false, i);
        } else {
        ...
        return WordAttributes(probability, unigramProbabilityEntry.isBlacklisted(),
                ...
                unigramProbabilityEntry.isPossiblyOffensive(), mHasHistoricalInfo
                                                               ? probabilityEntry.getHistoricalInfo()->getTimestamp() : NOT_A_TIMESTAMP,
                                                               mHasHistoricalInfo ? (i + 1) : NOT_A_NGRAM_TYPE);
    }
...
const int LanguageModelDictContent::getWordScore(const HistoricalInfo * const historicalInfo, const int contextCount, const bool isPersonalityDict, const int contextNum) const {
    const NgramType ngramType = NgramUtils::getNgramTypeFromWordCount(contextNum + 1);
    ...
    const int decayedProbability = DynamicLanguageModelProbabilityUtils::getDecayedProbability(encodedRawProbability, *historicalInfo);
    const int backoffProbability = DynamicLanguageModelProbabilityUtils::backoff(
