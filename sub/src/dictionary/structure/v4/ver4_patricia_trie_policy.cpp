...
int Ver4PatriciaTriePolicy::getCodePointsAndReturnCodePointCount(const int wordId,
                                                                 const int maxCodePointCount, int *const outCodePoints, int initialCount, const WordIdArrayView prevWordIds, WordAttributes *const attributes) const {
    ...
    if(initialCount == 0){
        ...
        initialCount = readingHelper.getCodePointsAndReturnCodePointCount(
                maxCodePointCount, outCodePoints);
    }

    if(!prevWordIds.empty()){
        int secondWordId = getSecondWord(wordId, prevWordIds, attributes);
        if (secondWordId != NOT_A_WORD_ID) {
            int secondCodePoints[MAX_WORD_LENGTH];
            const int ptNodePosSecond =
                    mBuffers->getTerminalPositionLookupTable()->getTerminalPtNodePosition(secondWordId);
            readingHelper.initWithPtNodePos(ptNodePosSecond);
            const int codePointCountSecond = readingHelper.getCodePointsAndReturnCodePointCount(
                    maxCodePointCount, secondCodePoints);
            // there is a blank between them.
            if (initialCount + codePointCountSecond <= MAX_WORD_LENGTH - 1) {
                outCodePoints[initialCount] = KEYCODE_SPACE;
                for (int i = 0; i < codePointCountSecond; i++) {
                    outCodePoints[initialCount + i + 1] = secondCodePoints[i];
                }
                return initialCount + codePointCountSecond + 1;
            }
        }
    }
    if (readingHelper.isError()) {
...
const WordAttributes Ver4PatriciaTriePolicy::getWordAttributesInContext(
    ...
    const WordAttributes Ver4PatriciaTriePolicy::getWordAttributesInContextForPredication(
            const WordIdArrayView prevWordIds, const int wordId,
            MultiBigramMap *const multiBigramMap) const {
        if (wordId == NOT_A_WORD_ID) {
            return WordAttributes();
...
int Ver4PatriciaTriePolicy::getProbabilityOfWord(const WordIdArrayView prevWordIds,
    ...
    int Ver4PatriciaTriePolicy::getProbabilityOfWordForPredication(const WordIdArrayView prevWordIds,
                                                                   const int wordId) const {
        if (wordId == NOT_A_WORD_ID || prevWordIds.contains(NOT_A_WORD_ID)) {
            return NOT_A_PROBABILITY;
        }
...
const WordProperty Ver4PatriciaTriePolicy::getWordProperty(
    ...
    for (const auto entry : languageModelDictContent->exportAllNgramEntriesRelatedToWord(
        ...
        const int codePointCount = getCodePointsAndReturnCodePointCount(entry.getTargetWordId(),
                MAX_WORD_LENGTH, ngramTargetCodePoints, 0, WordIdArrayView(), NULL);
        ...
        for (size_t i = 0; i < prevWordIds.size(); ++i) {
            ngramPrevWordsCodePointCount[i] = getCodePointsAndReturnCodePointCount(prevWordIds[i],
                       MAX_WORD_LENGTH, ngramPrevWordsCodePoints[i], 0, WordIdArrayView(), NULL);
...
int Ver4PatriciaTriePolicy::getNextWordAndNextToken(const int token, int *const outCodePoints,
    ...
    *outCodePointCount = getCodePointsAndReturnCodePointCount(ptNodeParams.getTerminalId(),
            MAX_WORD_LENGTH, outCodePoints, 0, WordIdArrayView(), NULL);
...
int Ver4PatriciaTriePolicy::getSecondWord(int targetWordId, WordIdArrayView prevWordIds, WordAttributes *const wordAttributes)const {
    // only history dictionary do this
    WordIdArray<MAX_PREV_WORD_COUNT_FOR_N_GRAM> prevWordIdArray;
    prevWordIdArray[0] = targetWordId;
    size_t size = prevWordIds.size();
    for (size_t i = 0; i < size; ++i) {
        if (i != 0) {
            prevWordIdArray[i] = prevWordIds[i - 1];
        }
    }
    WordIdArrayView prevWordsIdForSecondWord = WordIdArrayView::fromArray(prevWordIdArray);
    // get the max count second word in new context.
    int chooseWordId = NOT_A_WORD_ID;
    int chooseCount = 0;
    int chooseTimestamp = NOT_A_TIMESTAMP;
    HistoricalInfo* chooseOne;
    const auto languageModelDictContent = mBuffers->getLanguageModelDictContent();
    for (const auto entry: languageModelDictContent->getProbabilityEntries(
            prevWordsIdForSecondWord)) {
        const ProbabilityEntry &probabilityEntryTmp = entry.getProbabilityEntry();
        if (probabilityEntryTmp.hasHistoricalInfo()) {
            const HistoricalInfo *tmpInfo = probabilityEntryTmp.getHistoricalInfo();
            if (tmpInfo->isValid()) {
                if (tmpInfo->getCount() > chooseCount ||
                    (tmpInfo->getCount() == chooseCount &&
                     tmpInfo->getTimestamp() > chooseTimestamp)) {
                    chooseTimestamp = tmpInfo->getTimestamp();
                    chooseCount = tmpInfo->getCount();
                    chooseWordId = entry.getWordId();
                    chooseOne = (HistoricalInfo *) tmpInfo;
                }
            }
        }
    }
    if(chooseWordId != NOT_A_WORD_ID && wordAttributes){
        // judge whether phrase should be show by comparing the pure count(just count for this phrase, exclude nGram before them) of phrase is bigger than a threshold.
        const ProbabilityEntry phraseEntry = languageModelDictContent->getNgramProbabilityEntry(
                prevWordsIdForSecondWord.limit((size_t)(1)), chooseWordId);
        if(phraseEntry.getHistoricalInfo()->getCount() >= 2){
            int ngramNum = wordAttributes->getNgramType() - 1;
            if(ngramNum >= MAX_PREV_WORD_COUNT_FOR_N_GRAM){
                ngramNum--;
            }
            // rescore because word becomes words.
            const ProbabilityEntry prevWordProbabilityEntry = languageModelDictContent->getNgramProbabilityEntry(
                    prevWordIds.skip(1 /* n */).limit((size_t)(ngramNum - 1)), prevWordIds[0]);
            int contextCount = prevWordProbabilityEntry.getHistoricalInfo()->getCount();
            int realScore = languageModelDictContent->getWordScore(chooseOne, contextCount, false, ngramNum);
            wordAttributes->rescore(realScore);
            return chooseWordId;
        }
    }
    return NOT_A_WORD_ID;
