...
#include <utils/algorithm_switch.h>
...
void Dictionary::NgramListenerForPrediction::onVisitEntry(const int ngramProbability,
    ...
    if(mNgramContext == nullptr || mDictStructurePolicy == nullptr ||
            mSuggestionResults == nullptr) {
        return;
    }
    ...
    bool useNewAlgorithm = AlgorithmSwitch::getInstance().isUseNewAlgorithm();
    const WordAttributes wordAttributes = useNewAlgorithm ? mDictStructurePolicy->getWordAttributesInContextForPredication(
            mPrevWordIds, targetWordId, nullptr) : mDictStructurePolicy->getWordAttributesInContext(
            mPrevWordIds, targetWordId, nullptr);
    if (wordAttributes.getProbability() == NOT_A_PROBABILITY) {
        return;
    }
    int targetWordCodePoints[MAX_WORD_LENGTH];
    if(wordAttributes.getTimestamp() == NOT_A_TIMESTAMP){
        const int codePointCount = mDictStructurePolicy->getCodePointsAndReturnCodePointCount(
                targetWordId, MAX_WORD_LENGTH, targetWordCodePoints, 0, WordIdArrayView(), NULL);
        if (codePointCount <= 0) {
            return;
        }
        ...
        const int codePointCount = mDictStructurePolicy->getCodePointsAndReturnCodePointCount(
                targetWordId, MAX_WORD_LENGTH, targetWordCodePoints, 0, mPrevWordIds, (WordAttributes *const)(&wordAttributes));
        if (codePointCount <= 0) {
            return;
        }
        mSuggestionResults->addPrediction(targetWordCodePoints, codePointCount,
                                          wordAttributes.getProbability(), wordAttributes.getTimestamp());
    }
...
int Dictionary::getNgramProbability(const NgramContext *const ngramContext,
    ...
    TimeKeeper::setCurrentTime();
    if (!mDictionaryStructureWithBufferPolicy) {
    ...
    const DictionaryStructureWithBufferPolicy * dictionaryStructureWithBufferPolicy = getDictionaryStructurePolicy();
    if (!dictionaryStructureWithBufferPolicy) {
        return NOT_A_PROBABILITY;
    }
    if (!ngramContext) {
        return dictionaryStructureWithBufferPolicy->getProbabilityOfWord(WordIdArrayView(), wordId);
    }
    WordIdArray<MAX_PREV_WORD_COUNT_FOR_N_GRAM> prevWordIdArray;
    const WordIdArrayView prevWordIds = ngramContext->getPrevWordIds(
            mDictionaryStructureWithBufferPolicy.get(), &prevWordIdArray,
            true /* tryLowerCaseSearch */);
    return dictionaryStructureWithBufferPolicy->getProbabilityOfWord(prevWordIds, wordId);
}
