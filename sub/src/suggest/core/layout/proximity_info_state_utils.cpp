...
/* static */ float ProximityInfoStateUtils::getMostProbableString(
        ...
        const std::vector<int> *const sampledInputXs,
        const std::vector<int> *const sampledInputYs,
        int *const codePointBuf,std::vector<std::vector<int>> *sampledSearchKeyVectors,
        int *const codePointBufX,
        int *const codePointBufY) {
    ASSERT(sampledInputSize >= 0);
    memset(codePointBuf, 0, sizeof(codePointBuf[0]) * MAX_WORD_LENGTH);
    memset(codePointBufX, 0, MAX_WORD_LENGTH);
    memset(codePointBufY, 0, MAX_WORD_LENGTH);
    int index = 0;
    ...
    for (int i = 0; i < sampledInputSize && index < MAX_WORD_LENGTH - 1; ++i) {
        ...
        std::unordered_map<float, int> tempProbiliry;
        ...
        if (character != NOT_AN_INDEX) {
            ...
            codePointBufX[index] = (*sampledInputXs)[i];
            codePointBufY[index] = (*sampledInputYs)[i];
            tempProbiliry[minLogProbability] = character;
            index++;
            flag = true;
        }
        ...
        if(flag){
            flag = false;
            for (std::unordered_map<float , int>::const_iterator it = tempProbiliry.begin();
                 it != tempProbiliry.end(); ++it) {

                const int codePoint = proximityInfo->getCodePointOf(it->second);
                if (codePoint == NOT_A_CODE_POINT) {
                    continue;
                }
                temps.push_back(codePoint);
            }
        }
    }
    sampledSearchKeyVectors->resize(temps.size());
    for(size_t i =0; i<temps.size();i++){
        std::vector<int> *searchKeyVector = &(*sampledSearchKeyVectors)[i];
        searchKeyVector->push_back(temps.at(i));
    }
