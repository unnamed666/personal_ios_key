...
- (void)setThumbImage:(UIImage *)thumbImage
{
    if (_thumbImage != thumbImage) {
        ...
        [_thumbImageView setFrame:(CGRect){ { 0.0f, self.thumbOffsetY }, {self.bounds.size.height, self.bounds.size.height} }];
        [self createMasksForLockPosition];
...
- (CGFloat)valueAtThumbPosition
{
    ...
    if (range == 0) {
        kLog(@"TTSwitch can't get value at thumb position because you are going to have a bad time dividing by zero.");
        return 0.0f;
    }
