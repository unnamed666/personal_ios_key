...
class SuggestOptions{
 public:
    ...
    AK_FORCE_INLINE bool isGestureLogOpen() const {
        return getBoolOption(IS_GESTURE_LOG_OPEN);
    }
    ...
    AK_FORCE_INLINE int getDictOldVersion() const {
        return getIntOption(DICT_OLD_VERSION);
    }
 ...
 private:
    ...
    static const int IS_GESTURE_LOG_OPEN = 5;
    static const int DICT_OLD_VERSION = 6;
    // Additional features options are stored after the other options and used as setting values of
    // experimental features.
    static const int ADDITIONAL_FEATURES_OPTIONS = 7;
