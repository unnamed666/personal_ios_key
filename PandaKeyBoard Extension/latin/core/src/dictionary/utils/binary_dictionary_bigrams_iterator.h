...
class BinaryDictionaryBigramsIterator {
 public:
    ...
    AK_FORCE_INLINE void next() {
        mBigramsStructurePolicy->getNextBigram(&mBigramPos, &mProbability, &mHasNext, &mPos, true);
    }

    AK_FORCE_INLINE void nextBigram() {
        mBigramsStructurePolicy->getNextBigram(&mBigramPos, &mProbability, &mHasNext, &mPos, false);
    }
