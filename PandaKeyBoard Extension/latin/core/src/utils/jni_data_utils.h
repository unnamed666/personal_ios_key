...
namespace latinime {
    ...
    public:
        ...
        static void outputCodePoints(unsigned short intArrayToOutputCodePoints[], const int start,
                                     const int maxLength, const int *const codePoints, const int codePointCount,
                                     const bool needsNullTermination) {
            const int codePointBufSize = std::min(maxLength, codePointCount);
            unsigned short outputCodePonts[codePointBufSize];
            int outputCodePointCount = 0;
            for (int i = 0; i < codePointBufSize; ++i) {
                const int codePoint = codePoints[i];
                int codePointToOutput = codePoint;
            ...
            memcpy(intArrayToOutputCodePoints + start, outputCodePonts  , outputCodePointCount * sizeof(outputCodePonts[0]));
            if (needsNullTermination && outputCodePointCount < maxLength) {
                memcpy(intArrayToOutputCodePoints + start + outputCodePointCount, &CODE_POINT_NULL , sizeof(CODE_POINT_NULL));
            }
        }
        ...
        static void putIntToArray(int array[], const int index, const int value) {
//            memcpy(array, &value + index , 1);
            array[index] = value;
        }
    ...
    private:
        ...
        static const unsigned short CODE_POINT_REPLACEMENT_CHARACTER;
        static const unsigned short CODE_POINT_NULL;
    };
} // namespace latinime
