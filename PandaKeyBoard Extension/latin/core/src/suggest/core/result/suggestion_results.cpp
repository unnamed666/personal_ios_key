...
void SuggestionResults::outputSuggestions(JNIEnv *env, jintArray outSuggestionCount,
        ...
        jfloatArray outWeightOfLangModelVsSpatialModel, jintArray outTimestampsArray) {
    int outputIndex = 0;
    while (!mSuggestedWords.empty()) {
        ...
        JniDataUtils::putIntToArray(env, outTimestampsArray, outputIndex, suggestedWord.getTimestamp());
    ...
    void SuggestionResults::outputSuggestions(int * outSuggestionCount,
                                              unsigned short outCodePointsArray[],
                                              int outScoresArray[],
                                              int outSpaceIndicesArray[],
                                              int outTypesArray[],
                                              int outAutoCommitFirstWordConfidenceArray[],
                                              float *outWeightOfLangModelVsSpatialModel,
                                              int outTimestampsArray[]){
        int outputIndex = 0;
        while (!mSuggestedWords.empty()) {
            const SuggestedWord &suggestedWord = mSuggestedWords.top();
            suggestedWord.getCodePointCount();
            const int start = outputIndex * MAX_WORD_LENGTH;
            JniDataUtils::outputCodePoints(outCodePointsArray, start,
                                           MAX_WORD_LENGTH /* maxLength */, suggestedWord.getCodePoint(),
                                           suggestedWord.getCodePointCount(), true /* needsNullTermination */);
            JniDataUtils::putIntToArray(outScoresArray, outputIndex, suggestedWord.getScore());
            JniDataUtils::putIntToArray(outTimestampsArray, outputIndex, suggestedWord.getTimestamp());
            JniDataUtils::putIntToArray(outSpaceIndicesArray, outputIndex,
                                        suggestedWord.getIndexToPartialCommit());
            JniDataUtils::putIntToArray(outTypesArray, outputIndex, suggestedWord.getType());
            if (mSuggestedWords.size() == 1) {
                JniDataUtils::putIntToArray(outAutoCommitFirstWordConfidenceArray, 0 /* index */,
                                            suggestedWord.getAutoCommitFirstWordConfidence());
            }
        ...
        *outSuggestionCount = outputIndex;
        *outWeightOfLangModelVsSpatialModel = mWeightOfLangModelVsSpatialModel;
...
void SuggestionResults::addPrediction(const int *const codePoints, const int codePointCount,
    ...
    addSuggestion(codePoints, codePointCount, probability, Dictionary::KIND_PREDICTION,
                  NOT_AN_INDEX, NOT_A_FIRST_WORD_CONFIDENCE, timestamp);
}
...
void SuggestionResults::addSuggestion(const int *const codePoints, const int codePointCount,
                                      const int score, const int type, const int indexToPartialCommit,
                                      const int autocimmitFirstWordConfindence, const int timestamp) {
    if (codePointCount <= 0 || codePointCount > MAX_WORD_LENGTH) {
        // Invalid word.
        AKLOGE("Invalid word is added to the suggestion results. codePointCount: %d",
               codePointCount);
    ...
    if (getSuggestionCount() >= mMaxSuggestionCount) {
        const SuggestedWord &mWorstSuggestion = mSuggestedWords.top();
        if (score > mWorstSuggestion.getScore() ||
            (score == mWorstSuggestion.getScore() && timestamp > mWorstSuggestion.getTimestamp()) ||
            (score == mWorstSuggestion.getScore() && timestamp == mWorstSuggestion.getTimestamp()
             && codePointCount < mWorstSuggestion.getCodePointCount())) {
            mSuggestedWords.pop();
        } else {
    ...
    mSuggestedWords.push(SuggestedWord(codePoints, codePointCount, score, type,
                                       indexToPartialCommit, autocimmitFirstWordConfindence, timestamp));
}
