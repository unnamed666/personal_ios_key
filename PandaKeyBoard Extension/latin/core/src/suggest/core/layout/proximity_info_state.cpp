    ...
    if (mSampledInputSize > 0) {
        ...
        if (isGeometric) {
            ...
            mMostProbableStringProbability = ProximityInfoStateUtils::getMostProbableString(
                    mProximityInfo, mSampledInputSize, &mCharProbabilities, &mSampledInputXs, &mSampledInputYs,mMostProbableString,&mSampledSearchKeyVectors2,mMostProbableStringX, mMostProbableStringY);
...
ProximityType ProximityInfoState::getProximityTypeG(const int index, const int codePoint,int *proximityIndex) const {
    if (!isUsed()) {
        return UNRELATED_CHAR;
    }

    const int sampledSearchKeyVectorsSize = static_cast<int>(mSampledSearchKeyVectors2.size());
    ...
    for (int i = 0; i < static_cast<int>(mSampledSearchKeyVectors2[index].size()); ++i) {
        if (mSampledSearchKeyVectors2[index][i] == lowerCodePoint
            || mSampledSearchKeyVectors2[index][i] == baseLowerCodePoint) {
            return MATCH_CHAR;
        }
    }
...
float ProximityInfoState::getMostProbableString(int *const codePointBuf, int size) const {
    memmove(codePointBuf, mMostProbableString, sizeof(int) * size);
...
float ProximityInfoState::getProbability(const int index, const int keyIndex) const {
    ...
    int ProximityInfoState::getMostProbableStringSize() {
        int size = 0;
        for(int i = 0; i < MAX_WORD_LENGTH; i++) {
            if(mMostProbableString[i] > 0) {
                size++;
            }
        }
    ...
    void ProximityInfoState::getMostProbableStringXs(int *const xBuf, int size) {
        memmove(xBuf, mMostProbableStringX, sizeof(int)* size);
    }

    void ProximityInfoState::getMostProbableStringYs(int *const yBuf, int size) {
        memmove(yBuf, mMostProbableStringY, sizeof(int)* size);
    }

    void ProximityInfoState::getSampledXs(int *const xBuf) {
        memmove(xBuf, mSampledInputXs.data(), sizeof(int)*mSampledInputSize);
    }

    void ProximityInfoState::getSampledYs(int *const yBuf) {
        memmove(yBuf, mSampledInputYs.data(), sizeof(int)*mSampledInputSize);
    }
