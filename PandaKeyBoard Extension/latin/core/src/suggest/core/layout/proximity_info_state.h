...
class ProximityInfoState {
 public:
    ...
    AK_FORCE_INLINE ProximityInfoState()
            : mProximityInfo(nullptr), mMaxPointToKeyLength(0.0f), mAverageSpeed(0.0f),
              ...
              mCharProbabilities(), mSampledSearchKeySets(), mSampledSearchKeyVectors(),mSampledSearchKeyVectors2(),
        ...
        memset(mMostProbableString, 0, sizeof(mMostProbableStringX));
        memset(mMostProbableString, 0, sizeof(mMostProbableStringY));
}
    ...
    ProximityType getProximityTypeG(const int index, const int codePoint,int *proximityIndex=0) const;
    ...
    float getMostProbableString(int *const codePointBuf, int size) const;
    ...
    int getMostProbableStringSize();
    void getMostProbableStringXs(int *const xBuf, int size);
    void getMostProbableStringYs(int *const yBuf, int size);
    void getSampledXs(int *const xBuf);
    void getSampledYs(int *const yBuf);
    void getOriginalCodePoint(int *const codeBuf, int size);
 ...
 private:
    ...
    std::vector<std::vector<int>> mSampledSearchKeyVectors2;
    ...
    int mMostProbableStringX[MAX_WORD_LENGTH];
    int mMostProbableStringY[MAX_WORD_LENGTH];
};
} // namespace latinime
