...
template <typename T>
    static AK_FORCE_INLINE void safeGetOrFillZeroArrayRegion(const T jArray[],int len, T buffer[]) {
        if (jArray && buffer) {
            memcpy(buffer, jArray, len * sizeof(jArray[0]));
        } else if (buffer) {
            memset(buffer, 0, len * sizeof(buffer[0]));
        }
    }
//    
//    static AK_FORCE_INLINE void safeGetOrFillZeroIntArrayRegion(int jArray[],int len, int buffer[]) {
//        if (jArray && buffer) {
//            memcpy(buffer, jArray, len);
//        } else if (buffer) {
//            memset(buffer, 0, len * sizeof(buffer[0]));
//        }
//    }
//    
//    static AK_FORCE_INLINE void safeGetOrFillZeroFloatArrayRegion(float  jArray[], int len, float buffer[]) {
//        if (jArray && buffer) {
//            memcpy(buffer, jArray, len);
//        } else if (buffer) {
//            memset(buffer, 0, len * sizeof(buffer[0]));
...
ProximityInfo::ProximityInfo(const int keyboardWidth, const int keyboardHeight,
                                 const int gridWidth, const int gridHeight, const int mostCommonKeyWidth,
                                 const int mostCommonKeyHeight, const int proximityChars[], const int keyCount,
                                 const int  keyXCoordinates[], const int keyYCoordinates[],
                                 const int keyWidths[], const int keyHeights[], const int keyCharCodes[],
    ...
    mLowerCodePointToKeyMap(){
        ...
        safeGetOrFillZeroArrayRegion( proximityChars, GRID_WIDTH * GRID_HEIGHT * MAX_PROXIMITY_CHARS_SIZE, mProximityCharsArray);
        safeGetOrFillZeroArrayRegion( keyXCoordinates, KEY_COUNT, mKeyXCoordinates);
        safeGetOrFillZeroArrayRegion( keyYCoordinates, KEY_COUNT, mKeyYCoordinates);
        safeGetOrFillZeroArrayRegion( keyWidths, KEY_COUNT, mKeyWidths);
        safeGetOrFillZeroArrayRegion( keyHeights, KEY_COUNT, mKeyHeights);
        safeGetOrFillZeroArrayRegion( keyCharCodes, KEY_COUNT, mKeyCodePoints);
        safeGetOrFillZeroArrayRegion( sweetSpotCenterXs, KEY_COUNT, mSweetSpotCenterXs);
        safeGetOrFillZeroArrayRegion( sweetSpotCenterYs, KEY_COUNT, mSweetSpotCenterYs);
        safeGetOrFillZeroArrayRegion( sweetSpotRadii, KEY_COUNT, mSweetSpotRadii);
        initializeG();
}
