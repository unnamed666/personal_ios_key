...
def build_cmcm_task(t):
    t[u'groupid'] = 'group.com.cheetah.keyboard.theme.emoji';
    t[u'teamid'] = 'VRGS6G42JR';
    ...
    buildconfig_root = os.path.join(t[u'project_path'], 'buildconfigRelease')

    print "Process adhoc provisioning profiles..."
    user_provision_root = os.path.expanduser('~/Library/MobileDevice/Provisioning Profiles')

    array = ('adhoccomcheetahkeyboardthemeemoji.mobileprovision','adhoccomcheetahkeyboardthemeemojiExtension.mobileprovision','adhoccomcheetahkeyboardthemeemojiiMessageE.mobileprovision','comcheetahkeyboardthemeemoji.mobileprovision','comcheetahkeyboardthemeemojiExtension.mobileprovision','comcheetahkeyboardthemeemojiiMessageE.mobileprovision')
    if os.path.exists(buildconfig_root) and os.path.isdir(buildconfig_root):
    ...
    if u'adhoc_ipa_name_format' in t:
        ...
        for k in t.keys():
            adhoc_ipa_name_format = adhoc_ipa_name_format.replace('$(%s)'%k,t[k])
        t[u'adhoc_ipa_name_format'] = adhoc_ipa_name_format
        build_result = export_xcarchive(archiveFilesPath, build_dir, os.path.join(buildconfig_root, "ad-hoc.plist"))
    print t[u'adhoc_ipa_name_format']
    if not build_result:
        sys.exit(1)
 
    shutil.move(os.path.join(build_dir, scheme) +".ipa",os.path.join(build_dir, adhoc_ipa_name_format))

    #创建app_ipa
    if u'dist_ipa_name_format' in t:
        dist_ipa_name_format = t[u'dist_ipa_name_format']
        for k in t.keys():
            dist_ipa_name_format = dist_ipa_name_format.replace('$(%s)'%k,t[k])
        t[u'dist_ipa_name_format'] = dist_ipa_name_format
        build_result = export_xcarchive(archiveFilesPath, build_dir, os.path.join(buildconfig_root, "app-store.plist"))
    print t[u'dist_ipa_name_format']
    if not build_result:
        sys.exit(1)
 
    shutil.move(os.path.join(build_dir, scheme) +".ipa",os.path.join(build_dir, dist_ipa_name_format))
    ...
    bundle_version = t[u'build_version']
    version_part = bundle_version.split('.')
    versiondir= version_part[0] + '.' + version_part[1] + '.' + version_part[2]
    archive_root = t[u'archive_root']
    if archive_root.startswith('ftp://'):
        print '*******ftp开始文件上传****'
        copydir_to_ftp( build_dir, archive_root, versiondir+"/"+ archive_name )
        print '*******ftp文件上传结束****'
    else:
        shutil.copytree(build_dir, os.path.join(archive_root, archive_name), True)
...
def modify_teamid(t):
    ...
    newString = re.sub(build_pattern, lambda m : 'DEVELOPMENT_TEAM = '+t[u'teamid']+';', rawString)

    build_pattern = r'''DevelopmentTeam[\s]*=[\s]*([A-Za-z0-9]*);'''
    newString = re.sub(build_pattern, lambda m :'DevelopmentTeam = '+t[u'teamid']+';', newString)
...
def modify_Groupid(t):
    filename = './KeyboardKit/Config/CMGroupDataManager.m'
    build_pattern = r'''static NSString \* kGroupId(.)*'''
    rawString = open(filename, "rb").read()
    newString = re.sub(build_pattern, lambda m : 'static NSString * kGroupId = @"'+t[u'groupid']+'";', rawString)
    fw = open(filename, "wb")
    if fw == None:
        raise ("file can't write")
...
def buildEnterprise_task(t):
    t[u'groupid'] = 'group.com.cheetah.keyboard';
    t[u'teamid'] = 'RKFT92L7M9';
    ...
    array = ('com cheetah keyboard Extension.mobileprovision','com cheetah keyboard.mobileprovision','XC com cheetah keyboard Extension.mobileprovision','XC com cheetah keyboard.mobileprovision')
    if os.path.exists(buildconfig_root) and os.path.isdir(buildconfig_root):
        for prefile in array:
            filepath = os.path.join(buildconfig_root, prefile)
            print filepath
