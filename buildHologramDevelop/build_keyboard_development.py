...
#蒲公英提供的 用户Key
uKey = '8764fd1cd95e31e72da851c22c6b5f1c'
#蒲公英提供的 API Key
_api_key = 'ef87b980effb3d7e33369afb860c6b7b'
...
def update_bundle_info(t):
    ...
    if 'info_plist_ex' in t:
        print 'info_plist in t'
        info_plist_path = os.path.join(project_path, t[u'info_plist_ex'])
...
def build_task(t):
    ...
    if os.path.exists(buildconfig_root) and os.path.isdir(buildconfig_root):
        ...
        Developmentprovision_filename = 'Cheetah_Keyboard_Hologram_Extension_DevPro.mobileprovision'
...
def modify_teamid(t):
    ...
    newString = re.sub(build_pattern, lambda m : '''DEVELOPMENT_TEAM = 4UX7S99WCL;''', rawString)

    build_pattern = r'''DevelopmentTeam[\s]*=[\s]*([A-Za-z0-9]*);'''
    newString = re.sub(build_pattern, lambda m : '''DevelopmentTeam = 4UX7S99WCL;''', newString)
...
def modify_Groupid(t):
    filename = 'CMGroupDataManager.m'
    build_pattern = r'''static NSString \* kGroupId(.)*'''
    rawString = open(filename, "rb").read()
    newString = re.sub(build_pattern, lambda m : '''static NSString * kGroupId = @"group.com.cmcm.cheetah.hologram.keyboard";''', rawString)
    fw = open(filename, "wb")
    if fw == None:
        raise ("file can't write")
...
#    modify_teamid(t)
    ...
    buildconfig_root = os.path.join(t[u'project_path'], 'buildHologramDevelop')

    print "Process enterprise provisioning profiles..."
    user_provision_root = os.path.expanduser('~/Library/MobileDevice/Provisioning Profiles')

    array = ('Cheetah_Keyboard_Hologram_DevPro.mobileprovision','Cheetah_Keyboard_Hologram_Extension_DevPro.mobileprovision')
    if os.path.exists(buildconfig_root) and os.path.isdir(buildconfig_root):
    ...
    print "app_path = " + app_path
    #创建ipa
    if u'development_ipa_name_format' in t:
        development_ipa_name_format = t[u'development_ipa_name_format']
        for k in t.keys():
            development_ipa_name_format = development_ipa_name_format.replace('$(%s)'%k,t[k])
        t[u'development_ipa_name_format'] = development_ipa_name_format
        build_result = export_xcarchive(archiveFilesPath, build_dir, os.path.join(buildconfig_root, "hologram-development.plist"))
    print t[u'development_ipa_name_format']
    if not build_result:
        sys.exit(1)
 
    shutil.move(os.path.join(build_dir, scheme) +".ipa",os.path.join(build_dir, development_ipa_name_format))
    ...
    params = {
        'uKey': uKey,
        '_api_key': _api_key,
        'file': open(os.path.join(build_dir, development_ipa_name_format), 'rb'),
        'publishRange': '2',
        'password': installPassword
