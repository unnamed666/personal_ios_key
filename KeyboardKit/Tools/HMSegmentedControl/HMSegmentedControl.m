...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                ...
                if (isImageInLineWidthText) {
                    ...
                    if (self.imagePosition == HMSegmentedControlImagePositionLeftOfText) {
                        if(self.frame.size.width == 320 && idx == 2){
                            // iPhone 5 竖屏 emoticon 界面 红点 超出屏幕
                        }else{
                ...
                xOffset += ([self.segmentWidthsArray[idx] floatValue] - imageWidth - stringWidth - self.textImageSpacing) * 0.5;
            ...
            if(self.redDotIndex&&self.redDotIndex.count>idx&&self.redDotIndex[idx]!=[NSNull null]&&[self.redDotIndex[idx] boolValue]){
                [self addRedDotWithRect:textRect];
            }
...
- (void)addRedDotWithRect:(CGRect)rect{
    CALayer *redDotLayer = [CALayer layer];
    redDotLayer.backgroundColor = [UIColor redColor].CGColor;
    redDotLayer.cornerRadius = 3;
    redDotLayer.frame = CGRectMake(CGRectGetMaxX(rect), CGRectGetMinY(rect), 6, 6);
    [self.scrollView.layer addSublayer:redDotLayer];
}
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        if (segment != self.selectedSegmentIndex && segment < sectionsCount) {
            ...
            //yanzhao add begin 业务需要 点击当前已经选择的也要把事件传出去
            if(self.isTouchEnabled){
                if (self.superview)
