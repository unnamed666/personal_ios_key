...
//#include "lat/lattice-functions.h"  // used only for GetLattice
// ex: only #include "lat/determinize-lattice-pruned.h" used, already in the header
...
bool LatticeFasterDecoder::GetRawLattice(Lattice *ofst,
  ...
  // First create all states.  // read: each tok corres to a state.
  ...
  // Now create all arcs.  // read: each forwardlink corres to an arc.
  for (int32 f = 0; f <= num_frames; f++) {
...
void LatticeFasterDecoder::AdvanceDecoding(DecodableInterface *decodable, int32 max_num_frames) {
...
BaseFloat LatticeFasterDecoder::ProcessEmitting(DecodableInterface *decodable) {
  ...
  // read: only estimate a next_cutoff
  ...
  for (Elem *e = final_toks, *e_tail; e != NULL; e = e_tail) {
	...
	// read: del from toks_ but keep all in active_toks_
    StateId state = e->key;
    Token *tok = e->val;
    ...
    if (tok->tot_cost <= cur_cutoff) {
      for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state);
        ...
        if (arc.ilabel != 0) {  // propagate..
          ...
          if (tot_cost > next_cutoff) continue;  // read: do not add this tok according to estimated next_cutoff
...
void LatticeFasterDecoder::ProcessNonemitting(BaseFloat cutoff) {
  ...
  while (!queue_.empty()) {
    ...
    // read: tok iter is within one frame.
