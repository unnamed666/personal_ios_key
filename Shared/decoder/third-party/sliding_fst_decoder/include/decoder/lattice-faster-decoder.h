...
//#include "util/stl-utils.h"  // ex: used in hash-list
...
//#include "fstext/fstext-lib.h"             // ex: comment this doesn't make any diff
#include "lat/determinize-lattice-pruned.h"  // ex: this is only used for GetLattice
...
struct LatticeFasterDecoderConfig {
  ...
  LatticeFasterDecoderConfig(): beam(100.0),
                                max_active(200),//std::numeric_limits<int32>::max()
//                                max_active(200),
                                min_active(3),
                                lattice_beam(50.0),//50 is 10*2--1ms
                                prune_interval(25),
                                determinize_lattice(true),
                                beam_delta(0.5),
                                hash_ratio(2.0),
                                prune_scale(0.1) { }
	// add to adjust decoder parm for ios5 or later.
  void InitParams(BaseFloat _beam,int32 _max_active,int32 _min_active,BaseFloat _lattice_beam,
	  ...
	  min_active = _min_active;
	  lattice_beam = _lattice_beam;

	  prune_interval = _prune_interval;
	  determinize_lattice = _determinize_lattice;
	  beam_delta = _beam_delta;
	  hash_ratio = _hash_ratio;
	  prune_scale = _prune_scale;
  }
  /*
  //synisis-original
  LatticeFasterDecoderConfig(): beam(100.0),
                                max_active(10),//std::numeric_limits<int32>::max()
//                                max_active(200),
                                min_active(3),
                                lattice_beam(50.0),//50 is 10*2--1ms
                                prune_interval(25),
