...
//#include "fstext/fstext-lib.h"  // ex: to deterministic-fst.h
...
namespace kaldi {
    ...
    public:
        ...
        void hello_message() {
            cout << "This is LatticeBiglmFasterDecoder." << endl;
        }
        
        
        void InitDecoding() {
            //    // InitDecoding from lattice-faster
            //    DeleteElems(toks_.Clear());
            //
            //    cost_offsets_.clear();
            //    ClearActiveTokens();
            ...
            // above is init decoding.
            
        }
        
        void ResetDecoding(StateId lm_state) {
            DeleteElems(toks_.Clear());
            ...
            PairId start_pair = ConstructPair(fst_.Start(), lm_state);
            active_toks_.resize(1);
            Token *start_tok = new Token(0.0, 0.0, NULL, NULL);
            active_toks_[0].toks = start_tok;
...
//                lm_state = PairToLmState(state_pair);
                Token *tok = e->val;
                BaseFloat final_cost = fst_.Final(state).Value();// + lm_diff_fst_->Final(lm_state).Value();
        ...
        inline StateId PropagateLm(StateId lm_state, Arc *arc) { // returns new LM state.
