...
DefaultCompactStore<Element, Unsigned>
  ...
  if (compactor.Size() == -1) {
    if ((hdr.GetFlags() & FstHeader::IS_ALIGNED) && !AlignInput(strm)) {
      LOG_FST(ERROR) << "DefaultCompactStore::Read: Alignment failed: "
                 << opts.source;
      return nullptr;
    }
    ...
    if (!strm || !data->states_region_) {
      LOG_FST(ERROR) << "DefaultCompactStore::Read: Read failed: " << opts.source;
      return nullptr;
    }
    data->states_ =
...
bool DefaultCompactStore<Element, Unsigned>::Write(
  ...
  if (states_) {
    if (opts.align && !AlignOutput(strm)) {
      LOG_FST(ERROR) << "DefaultCompactStore::Write: Alignment failed: "
                 << opts.source;
      return false;
    }
  ...
  strm.flush();
  if (!strm) {
    LOG_FST(ERROR) << "DefaultCompactStore::Write: Write failed: " << opts.source;
    return false;
  }
  return true;
}
...
bool CompactFst<A, Compactor, Unsigned, CompactStore, CacheStore>::WriteFst(
  ...
  if (first_pass_compactor.Size() == -1) {
    if (opts.align && !AlignOutput(strm)) {
      LOG_FST(ERROR) << "CompactFst::Write: Alignment failed: " << opts.source;
      return false;
    }
    Unsigned compacts = 0;
    for (StateIterator<FST> siter(fst); !siter.Done(); siter.Next()) {
  ...
  strm.flush();
  if (!strm) {
    LOG_FST(ERROR) << "CompactFst write failed: " << opts.source;
    return false;
  }
  return true;
}
