...
class Fst {
 public:
  ...
  static Fst<Arc> *Read(std::istream &strm, const FstReadOptions &opts) {
    ...
    if (!reader) {
      LOG_FST(ERROR) << "Fst::Read: Unknown FST type " << fst_type
  ...
  virtual bool Write(std::ostream &strm, const FstWriteOptions &opts) const {
    LOG_FST(ERROR) << "Fst::Write: No write stream method for " << Type()
               << " FST type";
    return false;
  }
  ...
  virtual bool Write(const string &filename) const {
    LOG_FST(ERROR) << "Fst::Write: No write filename method for " << Type()
               << " FST type";
    return false;
  }
 ...
 protected:
  bool WriteFile(const string &filename) const {
    if (!filename.empty()) {
      ...
      if (!strm) {
        LOG_FST(ERROR) << "Fst::Write: Can't open file: " << filename;
        return false;
      }
      bool val = Write(strm, FstWriteOptions(filename));
      if (!val) LOG_FST(ERROR) << "Fst::Write failed: " << filename;
      return val;
    } else {
...
class FstImpl {
 public:
  ...
  static bool UpdateFstHeader(const Fst<Arc> &fst, std::ostream &strm,
    ...
    if (!strm) {
      LOG_FST(ERROR) << "Fst::UpdateFstHeader: Write failed: " << opts.source;
      return false;
    }
...
bool FstImpl<Arc>::ReadHeader(std::istream &strm, const FstReadOptions &opts,
  ...
  if (FLAGS_v >= 2) {
    LOG_FST(INFO) << "FstImpl::ReadHeader: source: " << opts.source
  ...
  if (hdr->FstType() != type_) {
    LOG_FST(ERROR) << "FstImpl::ReadHeader: FST not of type " << type_
               << ": " << opts.source;
    return false;
  }
  if (hdr->ArcType() != Arc::Type()) {
    LOG_FST(ERROR) << "FstImpl::ReadHeader: Arc not of type " << Arc::Type()
               << ": " << opts.source;
    return false;
  }
  if (hdr->Version() < min_version) {
    LOG_FST(ERROR) << "FstImpl::ReadHeader: Obsolete " << type_
