 ...
 public:
  ...
  static ExpandedFst<Arc> *Read(std::istream &strm,
    ...
    if (!(hdr.Properties() & kExpanded)) {
      LOG_FST(ERROR) << "ExpandedFst::Read: Not an ExpandedFst: " << ropts.source;
      return nullptr;
    }
    const auto reader =
        FstRegister<Arc>::GetRegister()->GetReader(hdr.FstType());
    if (!reader) {
      LOG_FST(ERROR) << "ExpandedFst::Read: Unknown FST type \"" << hdr.FstType()
  ...
  static ExpandedFst<Arc> *Read(const string &filename) {
    if (!filename.empty()) {
      ...
      if (!strm) {
        LOG_FST(ERROR) << "ExpandedFst::Read: Can't open file: " << filename;
        return nullptr;
      }
