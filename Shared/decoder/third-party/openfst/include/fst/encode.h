 ...
 public:
  ...
  const Tuple *Decode(Label key) const {
    if (key < 1 || key > encode_tuples_.size()) {
      LOG_FST(ERROR) << "EncodeTable::Decode: Unknown decode key: " << key;
      return nullptr;
    }
...
bool EncodeTable<Arc>::Write(std::ostream &strm,
  ...
  strm.flush();
  if (!strm) {
    LOG_FST(ERROR) << "EncodeTable::Write: Write failed: " << source;
    return false;
  }
  return true;
}
...
EncodeTable<Arc> *EncodeTable<Arc>::Read(std::istream &strm,
  ...
  if (magic_number != kEncodeMagicNumber) {
    LOG_FST(ERROR) << "EncodeTable::Read: Bad encode table header: " << source;
    return nullptr;
  }
  uint32 flags;
  ReadType(strm, &flags);
  ...
  ReadType(strm, &size);
  if (!strm) {
    LOG_FST(ERROR) << "EncodeTable::Read: Read failed: " << source;
    return nullptr;
  }
...
class EncodeMapper {
 ...
 public:
  ...
  bool Write(const string &filename) const {
    ...
    if (!strm) {
      LOG_FST(ERROR) << "EncodeMap: Can't open file: " << filename;
      return false;
    }
