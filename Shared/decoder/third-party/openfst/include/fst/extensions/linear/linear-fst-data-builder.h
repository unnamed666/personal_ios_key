...
bool LinearFstDataBuilder<A>::AddWord(Label word,
  ...
  if (word == LinearFstData<A>::kStartOfSentence ||
    ...
    LOG_FST(WARNING) << "Ignored: adding boundary label: "
...
bool LinearFstDataBuilder<A>::AddWord(
  ...
  for (size_t i = 0; i < possible_output.size(); ++i) {
    ...
    if (output == LinearFstData<A>::kStartOfSentence ||
      ...
      LOG_FST(WARNING) << "Ignored: word = " << TranslateLabel(word, isyms_)
...
bool LinearFstDataBuilder<A>::AddWeight(size_t group,
  ...
  // Check well-formedness of boundary marks on the input.
  {
    ...
    if (start_in_middle) {
      LOG_FST(WARNING) << "Ignored: start-of-sentence in the middle of the input!";
      LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
      LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
      return false;
    }
    if (end_in_middle) {
      LOG_FST(WARNING) << "Ignored: end-of-sentence in the middle of the input!";
      LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
      LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
      return false;
    }
  ...
  // Check well-formedness of boundary marks on the output.
  {
    ...
    if (non_first_start) {
      LOG_FST(WARNING) << "Ignored: start-of-sentence not appearing "
    ...
    if (non_last_end) {
      LOG_FST(WARNING) << "Ignored: end-of-sentence not appearing "
...
bool FeatureGroupBuilder<A>::AddWeight(const std::vector<Label> &input,
  ...
  if (input.size() - num_input_start < future_size_) {
    LOG_FST(WARNING) << "Ignored: start-of-sentence in the future!";
    LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
    LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, fsyms_);
    return false;
  }
  if (num_input_start > 0 &&
      input.size() - future_size_ - num_input_start <
          output.size() - num_output_start) {
    LOG_FST(WARNING) << "Ignored: matching start-of-sentence with actual output!";
    LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
    LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
    return false;
  }
  if (num_output_start > 0 &&
      input.size() - future_size_ - num_input_start >
          output.size() - num_output_start) {
    LOG_FST(WARNING) << "Ignored: matching start-of-sentence with actual input!";
    LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
    LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
    return false;
  }
  ...
  if (num_input_end > future_size_ && num_input_end - future_size_ != 1) {
    LOG_FST(WARNING) << "Ignored: matching end-of-sentence with actual output!";
    LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
    LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
    return false;
  }
  if (num_output_end > 0 &&
    ...
    LOG_FST(WARNING) << "Ignored: matching end-of-sentence with actual input!";
    LOG_FST(WARNING) << "\tInput: " << JoinLabels(input, fsyms_);
    LOG_FST(WARNING) << "\tOutput: " << JoinLabels(output, osyms_);
    return false;
  }
...
typename A::Label GuessStartOrEnd(std::vector<typename A::Label> *sequence,
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (left && right) {
      ...
      LOG_FST(WARNING) << "Boundary label in the middle of the sequence! position: "
                   << i << "; boundary: " << boundary
