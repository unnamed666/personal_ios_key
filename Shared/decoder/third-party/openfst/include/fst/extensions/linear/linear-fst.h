...
class LinearTaggerFstImpl : public CacheImpl<A> {
 public:
  ...
  bool Write(std::ostream &strm,  // NOLINT
    ...
    data_->Write(strm);
    if (!strm) {
      LOG_FST(ERROR) << "LinearTaggerFst::Write: Write failed: " << opts.source;
      return false;
    }
    return true;
  }
...
class LinearTaggerFst : public ImplToFst<internal::LinearTaggerFstImpl<A>> {
 public:
  ...
  explicit LinearTaggerFst(const Fst<A> &fst)
    ...
    LOG_FST(FATAL) << "LinearTaggerFst: no constructor from arbitrary FST.";
  }
  ...
  static LinearTaggerFst<A> *Read(const string &filename) {
    if (!filename.empty()) {
      ...
      if (!strm) {
        LOG_FST(ERROR) << "LinearTaggerFst::Read: Can't open file: " << filename;
        return nullptr;
      }
  ...
  bool Write(const string &filename) const override {
    if (!filename.empty()) {
      ...
      if (!strm) {
        LOG_FST(ERROR) << "LinearTaggerFst::Write: Can't open file: " << filename;
        return false;
      }
...
class LinearClassifierFstImpl : public CacheImpl<A> {
 public:
  ...
  bool Write(std::ostream &strm, const FstWriteOptions &opts) const {
    ...
    if (!strm) {
      LOG_FST(ERROR) << "LinearClassifierFst::Write: Write failed: " << opts.source;
      return false;
    }
    return true;
  }
...
class LinearClassifierFst
 ...
 public:
  ...
  explicit LinearClassifierFst(const Fst<A> &fst)
      : ImplToFst<Impl>(std::make_shared<Impl>()) {
    LOG_FST(FATAL) << "LinearClassifierFst: no constructor from arbitrary FST.";
  }
  ...
  static LinearClassifierFst<A> *Read(const string &filename) {
    if (!filename.empty()) {
      std::ifstream strm(filename.c_str(),
                              std::ios_base::in | std::ios_base::binary);
      if (!strm) {
        LOG_FST(ERROR) << "LinearClassifierFst::Read: Can't open file: "
                   << filename;
        return nullptr;
      }
      return Read(strm, FstReadOptions(filename));
  ...
  bool Write(const string &filename) const override {
    if (!filename.empty()) {
      std::ofstream strm(filename.c_str(),
                               std::ios_base::out | std::ios_base::binary);
      if (!strm) {
        LOG_FST(ERROR) << "ProdLmFst::Write: Can't open file: " << filename;
        return false;
      }
      return Write(strm, FstWriteOptions(filename));
    } else {
