...
void AddVocab(const string &vocab, SymbolTable *isyms, SymbolTable *fsyms,
  ...
  if (!in) LOG_FST(FATAL) << "Can't open file: " << vocab;
  ...
  while (GetVocabRecord<Arc>(vocab, in, isyms, fsyms, osyms, &word,
    ...
    if (word == kNoLabel) {
      LOG_FST(WARNING) << "Ignored: boundary word: " << fields[0];
      continue;
    }
...
void AddModel(const string &model, SymbolTable *fsyms, SymbolTable *osyms,
  ...
  if (!in) LOG_FST(FATAL) << "Can't open file: " << model;
  string line;
  std::getline(in, line);
  if (!in) LOG_FST(FATAL) << "Empty file: " << model;
  size_t future_size;
  {
    std::istringstream strm(line);
    strm >> future_size;
    if (!strm) LOG_FST(FATAL) << "Can't read future size: " << model;
  }
  ...
  while (GetModelRecord<Arc>(model, in, fsyms, osyms, &input_labels,
    ...
    if (output_labels.empty())
      LOG_FST(FATAL) << "Empty output sequence in source " << model << ", line "
                 << num_line;
...
void AddModel(const string &model, SymbolTable *fsyms, SymbolTable *osyms,
  ...
  if (future_size != 0)
    LOG_FST(FATAL) << "Classifier model must have future size = 0; got "
  ...
  while (GetModelRecord<Arc>(model, in, fsyms, osyms, &input_labels,
    ...
    if (output_labels.size() != 1)
      LOG_FST(FATAL) << "Output not a single label in source " << model << ", line "
                 << num_line;
...
bool GetVocabRecord(const string &vocab, std::istream &strm,  // NOLINT
  ...
  if (fields.size() != 3)
    LOG_FST(FATAL) << "Wrong number of fields in source " << vocab << ", line "
               << num_line;
...
bool GetModelRecord(const string &model, std::istream &strm,  // NOLINT
  ...
  SplitByWhitespace(line, &fields);
  if (fields.size() != 3)
    LOG_FST(FATAL) << "Wrong number of fields in source " << model << ", line "
               << num_line;
