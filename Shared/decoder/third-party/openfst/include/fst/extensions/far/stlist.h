...
bool ReadSTListHeader(const string &filename, Header *header) {
  if (filename.empty()) {
    LOG_FST(ERROR) << "ReadSTListHeader: Can't read header from standard input";
    return false;
  }
  std::ifstream strm(filename, std::ios_base::in | std::ios_base::binary);
  if (!strm) {
    LOG_FST(ERROR) << "ReadSTListHeader: Could not open file: " << filename;
    return false;
  }
  int32 magic_number = 0;
  ...
  if (magic_number != kSTListMagicNumber) {
    LOG_FST(ERROR) << "ReadSTListHeader: Wrong file type: " << filename;
    return false;
  }
  if (file_version != kSTListFileVersion) {
    LOG_FST(ERROR) << "ReadSTListHeader: Wrong file version: " << filename;
    return false;
  }
  string key;
  ReadType(strm, &key);
  ...
  if (!strm) {
    LOG_FST(ERROR) << "ReadSTListHeader: Error reading file: " << filename;
    return false;
  }
  return true;
}
