...
bool FarIsomorphic(const string &filename1, const string &filename2,
  ...
  if (!reader1) {
    LOG_FST(ERROR) << "FarIsomorphic: Cannot open FAR file " << filename1;
    return false;
  }
  std::unique_ptr<FarReader<Arc>> reader2(FarReader<Arc>::Open(filename2));
  if (!reader2) {
    LOG_FST(ERROR) << "FarIsomorphic: Cannot open FAR file " << filename2;
    return false;
  }
  if (!begin_key.empty()) {
  ...
  for (; !reader1->Done() && !reader2->Done();
    ...
    if (key1 != key2) {
      LOG_FST(ERROR) << "FarIsomorphic: Mismatched keys " << key1 << " and "
                 << key2;
      return false;
    }
    if (!Isomorphic(*(reader1->GetFst()), *(reader2->GetFst()), delta)) {
      LOG_FST(ERROR) << "FarIsomorphic: FSTs for key " << key1
                 << " are not isomorphic";
      return false;
    }
  }
  if (!reader1->Done() || !reader2->Done()) {
    LOG_FST(ERROR) << "FarIsomorphic: Key "
