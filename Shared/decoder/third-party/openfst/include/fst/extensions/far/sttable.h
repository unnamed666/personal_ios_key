...
class STTableWriter {
 public:
  ...
  static STTableWriter<T, Writer> *Create(const string &filename) {
    if (filename.empty()) {
      LOG_FST(ERROR) << "STTableWriter: Writing to standard out unsupported.";
      return nullptr;
    }
...
class STTableReader {
 public:
  ...
  static STTableReader<T, Reader> *Open(const string &filename) {
    if (filename.empty()) {
      LOG_FST(ERROR) << "STTableReader: Operation not supported on standard input";
      return nullptr;
    }
...
bool ReadSTTableHeader(const string &filename, Header *header) {
  if (filename.empty()) {
    LOG_FST(ERROR) << "ReadSTTable: Can't read header from standard input";
    return false;
  }
  std::ifstream strm(filename, std::ios_base::in | std::ios_base::binary);
  if (!strm) {
    LOG_FST(ERROR) << "ReadSTTableHeader: Could not open file: " << filename;
    return false;
  }
  int32 magic_number = 0;
  ...
  if (magic_number != kSTTableMagicNumber) {
    LOG_FST(ERROR) << "ReadSTTableHeader: Wrong file type: " << filename;
    return false;
  }
  if (file_version != kSTTableFileVersion) {
    LOG_FST(ERROR) << "ReadSTTableHeader: Wrong file version: " << filename;
    return false;
  }
  int64 i = -1;
  ...
  if (strm.fail()) {
    LOG_FST(ERROR) << "ReadSTTableHeader: Error reading file: " << filename;
    return false;
  }
