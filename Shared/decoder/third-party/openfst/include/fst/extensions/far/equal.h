...
bool FarEqual(const string &filename1, const string &filename2,
  ...
  if (!reader1) {
    LOG_FST(ERROR) << "FarEqual: Could not open FAR file " << filename1;
    return false;
  }
  std::unique_ptr<FarReader<Arc>> reader2(FarReader<Arc>::Open(filename2));
  if (!reader2) {
    LOG_FST(ERROR) << "FarEqual: Could not open FAR file " << filename2;
    return false;
  }
  if (!begin_key.empty()) {
    ...
    if (!find_begin1 || !find_begin2) {
      ...
      if (!ret) {
        LOG_FST(ERROR) << "FarEqual: Key " << begin_key << " missing from "
  ...
  for (; !reader1->Done() && !reader2->Done();
    ...
    if (key1 != key2) {
      LOG_FST(ERROR) << "FarEqual: Mismatched keys " << key1 << " and " << key2;
      return false;
    }
    if (!Equal(*(reader1->GetFst()), *(reader2->GetFst()), delta)) {
      LOG_FST(ERROR) << "FarEqual: FSTs for key " << key1 << " are not equal";
      return false;
    }
  }
  if (!reader1->Done() || !reader2->Done()) {
    LOG_FST(ERROR) << "FarEqual: Key "
