 ...
 public:
  ...
  bool SingleDecode(const Var &index, Edge *output) {
    if (index >= decode_vector_.size()) {
      LOG_FST(ERROR) << "LempelZiv::SingleDecode: "
...
bool LempelZiv<Var, Edge, EdgeLessThan, EdgeEquals>::BatchDecode(
  ...
  for (typename std::vector<std::pair<Var, Edge>>::const_iterator it =
    ...
    if (temp_integer >= decode_vector_.size()) {
      LOG_FST(ERROR) << "LempelZiv::BatchDecode: "
                 << "Index exceeded the dictionary size";
      return false;
    } else {
...
bool Compressor<Arc>::Decompress(std::istream &strm, const string &source,
  ...
  if (magic_number != kCompressMagicNumber) {
    LOG_FST(ERROR) << "Decompress: Bad compressed Fst: " << source;
    ...
    if ((magic_number & kGzipMask) == kGzipMagicNumber) {
      LOG_FST(ERROR) << "Decompress: Fst appears to be compressed with Gzip, but "
...
bool Compress(const Fst<Arc> &fst, const string &file_name,
  ...
  if (gzip) {
    if (file_name.empty()) {
      ...
      gzfile.write(strm);
      if (!gzfile) {
        LOG_FST(ERROR) << "Compress: Can't write to file: stdout";
        return false;
      }
    } else {
      std::stringstream strm;
      ...
      if (!gzfile) {
        LOG_FST(ERROR) << "Compress: Can't open file: " << file_name;
        return false;
      }
      gzfile.write(strm);
      if (!gzfile) {
        LOG_FST(ERROR) << "Compress: Can't write to file: " << file_name;
        return false;
      }
    }
...
bool Decompress(const string &file_name, MutableFst<Arc> *fst,
  ...
  if (gzip) {
    if (file_name.empty()) {
      ...
      if (!gzfile) {
        LOG_FST(ERROR) << "Decompress: Can't read from file: stdin";
        return false;
      }
    } else {
      IGzFile gzfile(file_name.c_str());
      if (!gzfile) {
        LOG_FST(ERROR) << "Decompress: Can't open file: " << file_name;
        return false;
      }
      Decompress(*gzfile.read(), file_name, fst);
      if (!gzfile) {
        LOG_FST(ERROR) << "Decompress: Can't read from file: " << file_name;
        return false;
      }
    }
  } else if (file_name.empty()) {
