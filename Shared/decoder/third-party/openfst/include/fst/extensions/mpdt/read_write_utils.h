...
bool ReadLabelTriples(const string &filename,
  ...
  if (!fstrm) {
    LOG_FST(ERROR) << "ReadIntTriples: Can't open file: " << filename;
    return false;
  }
  ...
  while (fstrm.getline(line, kLineLen)) {
    ...
    if (col.size() != 3) {
      LOG_FST(ERROR) << "ReadLabelTriples: Bad number of columns, "
...
bool WriteLabelTriples(const string &filename,
  ...
  if (pairs.size() != assignments.size()) {
    LOG_FST(ERROR) << "WriteLabelTriples: Pairs and assignments of different sizes";
    return false;
  }
  std::ofstream fstrm(filename);
  if (!fstrm) {
    LOG_FST(ERROR) << "WriteLabelTriples: Can't open file: " << filename;
    return false;
  }
  for (size_t n = 0; n < pairs.size(); ++n)
    fstrm << pairs[n].first << "\t" << pairs[n].second << "\t" << assignments[n]
          << "\n";
  if (!fstrm) {
    LOG_FST(ERROR) << "WriteLabelTriples: Write failed: "
