...
VectorFstImpl<S> *VectorFstImpl<S>::Read(std::istream &strm,
  ...
  for (; hdr.NumStates() == kNoStateId || state < hdr.NumStates(); ++state) {
    ...
    ReadType(strm, &narcs);
    if (!strm) {
      LOG_FST(ERROR) << "VectorFst::Read: Read failed: " << opts.source;
      return nullptr;
    }
  ...
  if (hdr.NumStates() != kNoStateId && state != hdr.NumStates()) {
    LOG_FST(ERROR) << "VectorFst::Read: Unexpected end of file: " << opts.source;
    return nullptr;
  }
  return impl.release();
}
...
bool VectorFst<Arc, State>::WriteFst(const FST &fst, std::ostream &strm,
  ...
  strm.flush();
  if (!strm) {
    LOG_FST(ERROR) << "VectorFst::Write: Write failed: " << opts.source;
    return false;
  }
  if (update_header) {
