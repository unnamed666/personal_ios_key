...
bool UTF8StringToLabels(const string &str, std::vector<Label> *labels) {
  ...
  for (size_t i = 0; i < length;) {
    ...
    if ((c & 0x80) == 0) {
      ...
      if ((c & 0xc0) == 0x80) {
        LOG_FST(ERROR) << "UTF8StringToLabels: Continuation byte as lead byte";
        return false;
      }
      int count =
      ...
      while (count != 0) {
        if (i == length) {
          LOG_FST(ERROR) << "UTF8StringToLabels: Truncated UTF-8 byte sequence";
          return false;
        }
        char cb = data[i++];
        if ((cb & 0xc0) != 0x80) {
          LOG_FST(ERROR) << "UTF8StringToLabels: Missing/invalid continuation byte";
          return false;
        }
        code = (code << 6) | (cb & 0x3f);
        count--;
      ...
      if (code < 0) {
        ...
        LOG_FST(ERROR) << "UTF8StringToLabels: Invalid character found: " << c;
        return false;
      }
...
bool LabelsToUTF8String(const std::vector<Label> &labels, string *str) {
  ...
  for (size_t i = 0; i < labels.size(); ++i) {
    int32 code = labels[i];
    if (code < 0) {
      LOG_FST(ERROR) << "LabelsToUTF8String: Invalid character found: " << code;
      return false;
    } else if (code < 0x80) {
