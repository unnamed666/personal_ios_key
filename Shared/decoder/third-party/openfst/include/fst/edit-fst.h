...
class EditFstData {
 public:
  ...
  bool Write(std::ostream &strm, const FstWriteOptions &opts) const {
    ...
    if (!strm) {
      LOG_FST(ERROR) << "EditFstData::Write: Write failed: " << opts.source;
      return false;
    }
    return true;
  }
  ...
  void PrintMap() {
    for (auto map_it = external_to_internal_ids_.begin();
      ...
      LOG_FST(INFO) << "(external,internal)=(" << map_it->first << ","
                << map_it->second << ")";
    }
  }
...
EditFstData<A, WrappedFstT, MutableFstT>::Read(std::istream &strm,
  ...
  if (!strm) {
    LOG_FST(ERROR) << "EditFst::Read: read failed: " << opts.source;
    return nullptr;
  }
  return data;
}
...
class EditFstImpl : public FstImpl<A> {
 public:
  ...
  bool Write(std::ostream &strm, const FstWriteOptions &opts) const {
    ...
    strm.flush();
    if (!strm) {
      LOG_FST(ERROR) << "EditFst::Write: Write failed: " << opts.source;
      return false;
    }
    return true;
  }
