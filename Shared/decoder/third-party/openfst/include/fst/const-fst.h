...
ConstFstImpl<Arc, Unsigned> *ConstFstImpl<Arc, Unsigned>::Read(
  ...
  if ((hdr.GetFlags() & FstHeader::IS_ALIGNED) && !AlignInput(strm)) {
    LOG_FST(ERROR) << "ConstFst::Read: Alignment failed: " << opts.source;
    return nullptr;
  }
  ...
  if (!strm || !impl->states_region_) {
    LOG_FST(ERROR) << "ConstFst::Read: Read failed: " << opts.source;
    return nullptr;
  }
  impl->states_ =
...
bool ConstFst<Arc, Unsigned>::WriteFst(const FST &fst, std::ostream &strm,
  ...
  if (opts.align && !AlignOutput(strm)) {
    LOG_FST(ERROR) << "Could not align file during write after header";
    return false;
  }
  size_t pos = 0;
  size_t states = 0;
  ...
  hdr.SetNumArcs(pos);
  if (opts.align && !AlignOutput(strm)) {
    LOG_FST(ERROR) << "Could not align file during write after writing states";
  }
  for (StateIterator<FST> siter(fst); !siter.Done(); siter.Next()) {
  ...
  strm.flush();
  if (!strm) {
    LOG_FST(ERROR) << "ConstFst::WriteFst: write failed: " << opts.source;
    return false;
  }
  if (update_header) {
    ...
    if (hdr.NumStates() != num_states) {
      LOG_FST(ERROR) << "Inconsistent number of states observed during write";
      return false;
    }
    if (hdr.NumArcs() != num_arcs) {
      LOG_FST(ERROR) << "Inconsistent number of arcs observed during write";
      return false;
    }
  }
  return true;
