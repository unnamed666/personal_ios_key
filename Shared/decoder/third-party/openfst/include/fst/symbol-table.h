...
class SymbolTable {
 public:
  ...
  static SymbolTable *ReadText(const string &filename,
    ...
    if (!strm.good()) {
      LOG_FST(ERROR) << "SymbolTable::ReadText: Can't open file " << filename;
      return nullptr;
    }
  ...
  static SymbolTable *Read(const string& filename) {
    ...
    if (!strm.good()) {
      LOG_FST(ERROR) << "SymbolTable::Read: Can't open file " << filename;
      return nullptr;
    }
  ...
  bool Write(const string &filename) const {
    ...
    if (!strm.good()) {
      LOG_FST(ERROR) << "SymbolTable::Write: Can't open file " << filename;
      return false;
    }
    return Write(strm);
  }
  ...
  bool WriteText(const string &filename) const {
    ...
    if (!strm.good()) {
      LOG_FST(ERROR) << "SymbolTable::WriteText: Can't open file " << filename;
      return false;
    }
