...
#define FSTERROR() \
  (FLAGS_fst_error_fatal ? LOG_FST(FATAL) : LOG_FST(ERROR))
...
bool ReadIntPairs(const string &filename, std::vector<std::pair<I, I>> *pairs,
  ...
  if (!strm) {
    LOG_FST(ERROR) << "ReadIntPairs: Can't open file: " << filename;
    return false;
  }
  ...
  while (strm.getline(line, kLineLen)) {
    ...
    if (col.size() != 2) {
      LOG_FST(ERROR) << "ReadIntPairs: Bad number of columns, "
...
bool WriteIntPairs(const string &filename,
  ...
  if (!filename.empty()) {
    ...
    if (!*strm) {
      LOG_FST(ERROR) << "WriteIntPairs: Can't open file: " << filename;
      return false;
    }
  }
  ...
  if (!*strm) {
    LOG_FST(ERROR) << "WriteIntPairs: Write failed: "
