  ...
  if (region_.size != 0) {
    if (region_.mmap) {
      ...
      if (munmap(region_.mmap, region_.size) != 0) {
        LOG_FST(ERROR) << "Failed to unmap region: " << strerror(errno);
      }
    } else {
      if (region_.data) {
...
MappedFile *MappedFile::Map(std::istream *istrm, bool memorymap,
  ...
  if (memorymap && spos >= 0 && spos % kArchAlignment == 0) {
    ...
    if (fd != -1) {
      ...
      if (close(fd) == 0 && map != MAP_FAILED) {
        ...
        LOG_FST(INFO) << "Mapping of file failed: " << strerror(errno);
      }
    }
  }
  // If all else fails, reads from the file into the allocated buffer.
  if (memorymap) {
    LOG_FST(WARNING) << "File mapping at offset " << spos << " of file " << source
  ...
  while (size > 0) {
    ...
    if (!istrm->read(buffer, next_size)) {
      LOG_FST(ERROR) << "Failed to read " << next_size << " bytes at offset "
