...
SymbolTable *FstReadSymbols(const string &filename, bool input_symbols) {
  ...
  if (!in) {
    LOG_FST(ERROR) << "FstReadSymbols: Can't open file " << filename;
    return nullptr;
  }
  FstHeader hdr;
  if (!hdr.Read(in, filename)) {
    LOG_FST(ERROR) << "FstReadSymbols: Couldn't read header from " << filename;
    return nullptr;
  }
  if (hdr.GetFlags() & FstHeader::HAS_ISYMBOLS) {
    std::unique_ptr<SymbolTable> isymbols(SymbolTable::Read(in, filename));
    if (isymbols == nullptr) {
      LOG_FST(ERROR) << "FstReadSymbols: Couldn't read input symbols from "
                 << filename;
      return nullptr;
    }
  ...
  if (hdr.GetFlags() & FstHeader::HAS_OSYMBOLS) {
    ...
    if (osymbols == nullptr) {
      LOG_FST(ERROR) << "FstReadSymbols: Couldn't read output symbols from "
                 << filename;
      return nullptr;
    }
    if (!input_symbols) return osymbols.release();
  }
  LOG_FST(ERROR) << "FstReadSymbols: The file " << filename
